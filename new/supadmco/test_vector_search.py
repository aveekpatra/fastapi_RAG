...
COLLECTION_NAME = "czech_supreme_administrative_court"
...
def generate_test_queries(json_files, num_queries=5):
    ...
    # Real-world legal scenarios based on Czech Constitutional Court cases
    test_queries = [
        {
            'text': 'Můžu podat kasační stížnost, když jsem zmeškal lhůtu kvůli pozdnímu doručení rozhodnutí?',
            'description': 'Citizen asking about filing a cassation complaint after missing deadline'
        },
        {
            'text': 'Jak dlouho mám čas na podání návrhu po vyhlášení výsledků místního referenda?',
            'description': 'Question about procedural deadlines for local referendum challenges'
        },
        {
            'text': 'Politická strana nesplnila povinnost předložit finanční zprávu. Co se stane?',
            'description': 'Query about consequences of political party failing to submit financial reports'
        },
        {
            'text': 'Státní zástupce ztratil USB disk s citlivými údaji ze spisu. Jaká je odpovědnost?',
            'description': 'Question about disciplinary liability for data breach by prosecutor'
        },
        {
            'text': 'Je možné obnovit činnost politické strany po pozastavení kvůli nesplnění zákonných povinností?',
            'description': 'Query about reinstating suspended political party activity'
        },
        {
            'text': 'Kdo rozhoduje o kárném provinění státního zástupce a je možné se odvolat?',
            'description': 'Question about disciplinary proceedings and appeal rights for prosecutors'
        },
        {
            'text': 'Musí soud přijmout návrh podaný po 16:00 hodině v poslední den lhůty?',
            'description': 'Practical question about exact deadline timing for court submissions'
        },
        {
            'text': 'Může Nejvyšší správní soud rozhodovat o znovuobnovení činnosti politické strany?',
            'description': 'Jurisdictional question about Supreme Administrative Court powers'
        }
    ]
...
class VectorSearchTester:
    ...
    def test_query(self, query_info, query_num):
        ...
        # Check if results are relevant (score > 0.3 is generally good)
        relevant_results = 0
        ...
        for i, result in enumerate(results, 1):
            ...
            if score > 0.3:
                relevant_results += 1
                print(f"     ✅ RELEVANT (score > 0.3)")
            else:
                print(f"     ⚠️  Low relevance")
    ...
    def run_tests(self):
        ...
        print("when searching for relevant case law.\n")
        
        queries = generate_test_queries([], num_queries=8)
